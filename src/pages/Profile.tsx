  ...
  useEffect(() => {
    (async () => {
      const result = await query(
        'SELECT id, points, reason, earned_at FROM points_ledger WHERE player_id = ? ORDER BY earned_at DESC LIMIT 10',
        [playerId]
      );
      setHistory(result.rows as any[] || []);
    })();
  }, [playerId]);
...
const Profile = () => {
  ...
  const handleSave = async () => {
    if (!playerId) return;
    await query(
      'UPDATE players SET first_name = ?, last_name = ?, preferred_name = ? WHERE id = ?',
      [editForm.first_name, editForm.last_name, editForm.preferred_name || null, playerId]
    );
    setIsEditing(false);
    fetchProfile();
  };
