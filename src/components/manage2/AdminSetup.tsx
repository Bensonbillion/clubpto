...
import { SkillTier } from "@/types/openCourtManager";
import PlayerManager from "@/components/manage/PlayerManager";
import CsvImport from "@/components/manage/CsvImport";
  ...
  return (
    <div className="space-y-8 animate-fade-up">
      ...
      <div className="rounded-lg border border-border bg-card p-8 space-y-5">
        ...
        <div>
          <label className="text-sm uppercase tracking-widest text-muted-foreground mb-2 block">Session Name</label>
          <input
            type="text"
            value={(state.sessionConfig as any).sessionName || ""}
            onChange={(e) => setSessionConfig({ sessionName: e.target.value } as any)}
            placeholder="e.g. Wednesday Mississauga, Thursday Vaughan"
            className="w-full rounded-md border border-border bg-muted px-4 py-3 text-base text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-accent/30 min-h-[48px]"
          />
        </div>
        <div>
            ...
            {([1, 2] as const).map((count) => (
              <button
                ...
                className={`px-5 py-2.5 rounded-lg border-2 font-display text-base min-h-[48px] transition-all ${
                  (state.sessionConfig.courtCount || 1) === count
        ...
        <p className="text-sm text-muted-foreground sm:col-span-2">
          Games are doubles (2v2) played to 7 points (~7 min each). {Math.floor((state.sessionConfig.durationMinutes || 85) / 7)} game slots per court, {Math.floor((state.sessionConfig.durationMinutes || 85) / 7) * (state.sessionConfig.courtCount || 1)} total across {state.sessionConfig.courtCount || 1} court{(state.sessionConfig.courtCount || 1) > 1 ? "s" : ""}.
          <span className="block mt-1 text-accent">Open mode — all players can face any opponent regardless of tier. Tiers are for tracking only.</span>
        </p>
      </div>
