...
---
## 1. The Format
Everyone plays doubles but scores as an individual. Partners change every match. Players split into two **pools** by hidden tier — displayed only as **Court 1** and **Court 2**, never as skill labels. Play is **rolling**: a court finishes, the result goes in, the next four walk on. **(Amended — match format is a setup-time choice, per pool.)** A pool plays either **best of N games** (N = 3 or 5; a win needs ⌈N/2⌉ games, and the result records only the **games the losers took** — rendered "2–1", "3–2") or **a single game to a target** of 4–15 points (recorded as the loser's score — "7–4"). Nothing hard-codes a score alphabet: it is generated from N. The two courts can run different formats on the same night. Format is chosen at setup and **locks the moment a pool records its first result**; changing it after that means voiding the result and replaying it. Everyone plays the **exact same number of matches** (§3). Playoffs are built by the system from the standings — nobody picks anybody — and can be triggered **early** at any moment if the night is running long (§6).
...
1. **Select** — from players present, not on court, and below the match target, take the four with the fewest matches. **(Amended)** Wait is the elevator ONLY for never-played players and late arrivals; within an equal-matches group, selection prefers the four with the **fewest prior co-occurrences tonight** (partners or opponents, completed + active matches; voided excluded). Hard rule: never field an exact foursome that has already played together while any alternative foursome satisfying the hard constraints (present, below-target/fill-rule, back-to-back) exists. Deterministic: co-occurrence count, then playerId.
...
**Named trade of rolling play:** which four play *together* is driven by the queue (matches, then encounter variety), not by form. **(Amended)** Intra-wave order is explicitly NOT a fairness property — the wave property (max−min matches ≤ 1) and exact target convergence are the fairness guarantees, and they are unweakened. The pool already did the skill sorting and the 1+4/2+3 crossover balances every quartet — an acceptable trade for zero timers and courts that never wait on each other.
---
## 4. Scoring and Standings
**(Amended — differential is format-defined.)** Entry is **two taps in every format**: the winning pair, then the margin. The second row of buttons is generated from the format — two buttons at best of 3, three at best of 5, T buttons for a game to T.
...
F:    winners
```
Crossing keeps the two strong pairs apart until the final — the best match of the night by construction — while seeds 5–8 get a live shot at the upset. **(Amended)** Playoff matches default to the pool's own format, and the confirm screen can override it for the bracket alone — a pool that played games to 7 all night can still finish best of 5. Each bracket match carries the format it was created with, so entry and the log render it correctly even when the pool's format differs. Three matches, sequential on the pool's court. If the other court is free (its pool finished, or has no playoff), the two semis run in parallel there and the playoff takes two slots instead of three.
...
---
**(Amended — design language, STEP G.) The primary device is a PHONE** (390×844, one-handed, in a noisy room at night); the tablet is secondary and gets the same components larger. The reference is the Americano Padel app the club actually ran a session on: **one idea per screen**. The live screen is a SINGLE match card — court label quiet, four names large, two big score boxes that ARE the entry surface (tap the winning side, the format's score row appears inline beneath it, second tap records). Beneath it two quiet lines: who is next, who is resting. Then nothing. Completed matches live in a **swipeable pager** of the same card, which is also where corrections and voids happen — one mental model, not a card plus a log panel. Three bottom tabs — **Match / Players / Standings** — and a ⋮ for pace detail, playoff controls and session info; nothing operational lives only in ⋮. Setup is a **pad**: a name field with roster autocomplete, a row of config chips that each open a small bottom sheet, and one giant START. The whole default path — load, add names, start — must be doable in under a minute with one thumb. Our engine does what that app cannot (pools, exact targets, variety, the chain, the visible coin, playoffs, publish); this is that power wearing its simplicity.
...
**Simulation before it runs live:** run four scenarios. *Normal (16/12):* full night, assert everyone lands exactly on target, zero repeated partnerships while alternatives existed, no back-to-backs, top-8s ordered by the full chain. **(Amended)** Also: zero repeated foursomes while alternatives existed; every player meets at least 5 distinct others and the pool averages at least 7 (16@3); SOS resolves ties before any coin flip fires. *Variable split (14/14, target 4):* assert the setup screen rejects target 3 for a 14-pool, offers 4, and the night converges everyone to exactly 4. *Early cut:* trigger Court 2's playoff mid-wave (14 players at 3, 2 at 2), assert the W–L chain seeds correctly and the confirm data matches reality. **(Amended) *Single-game night (16 @ 3, game to 7):*** run the full night in single-game format through the production reducers with a JSON round-trip at every step; assert differential equals T − loser's score for every player on every match, that the chain orders correctly on those numbers, and that the coin count does not exceed the equivalent best-of-3 seed's (a 0–7 alphabet separates more finely than a 1–2 one). *Messy night:* one not-here swap on the opening match, one arrival after match 6, one departure after match 9 — assert the fill rule caps overage at one, the late arrival gets their catch-up back-to-back and correct eligibility, and standings stay coherent throughout.
